...
class AnalysisJob(Base):
    ...
    /api/leads enqueues) and app.workers.tasks.run_scheduled_collection (the
    Celery beat task that re-pulls Onionoo/MISP/HIBP on a fixed interval —
    see celery_app.py's beat_schedule) — a real row per run, not a
    fabricated history. task_id matches the Celery task id also returned to
    the API caller (GET /api/jobs/{task_id} reads live status from Celery's
    own result backend; this table is a separate, persistent history of the
    same runs, queryable after the Celery result TTL expires). CLI-driven
    analysis (scripts/ingest_and_attribute.py, ingest_evolution.py, etc.)
    calls run_full_analysis directly, not through Celery, so it does NOT
    appear here — this is honestly a history of async jobs, not of every
    pipeline run."""
    ...
    job_type: Mapped[str] = mapped_column(
        String(64)
    )  # infra_scan | stylometry | graph | wallet | reanalyze_all | scheduled_collection
